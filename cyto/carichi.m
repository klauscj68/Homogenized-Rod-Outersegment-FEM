...
q_th =     (1-theta_0)*k_hyd*(PDE_s/(1/2*nu*epsilon_0))*u_th_vol;
% termine f1, che dipende da v nel problema per u (ciclasi)
q_th = q_th - (1-theta_0)*(alpha_min+(alpha_max-alpha_min)*k_cyc^m_cyc./(k_cyc^m_cyc+v_th_vol.^m_cyc));
%q_th = q_th - (1-theta_0)*(alpha_min);
...
for d=1:n_sd
	...
	q_th =     2*k_hyd*PDE_s*u_th_sd;
	% fosfodiesterasi accesa
 q_th = q_th + k_st*E_st_th(:,:,d).*u_th_sd;
 %%%%%%%%q_th = q_th + k_st*E_st_th(:,:,d)*u_ss;
	% termine f1, che dipende da v nel problema per u (ciclasi)
	q_th = q_th - 2*(alpha_min+(alpha_max-alpha_min)*k_cyc^m_cyc./(k_cyc^m_cyc+v_th_sd.^m_cyc))*(1/2*nu*epsilon_0);
%%%	q_th = q_th - 2*(alpha_min)*(1/2*nu*epsilon_0);
...
q_th =     j_ex_sat/(Sigma_rod*B_ca*F)*v_th_os./(K_ex+v_th_os);
...
q_th = q_th - j_cg_max*f_ca/(2*Sigma_rod*B_ca*F)*u_th_os.^m_cg./(K_cg^m_cg+u_th_os.^m_cg);
